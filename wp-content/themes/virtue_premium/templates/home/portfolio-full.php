<div class="home-portfolio clearfix home-margin home-padding">
	<?php global $virtue_premium; if(!empty($virtue_premium['portfolio_full_title'])) {$port_full_title = $virtue_premium['portfolio_full_title']; } else { $port_full_title = '';} ?>
		<div class="clearfix"><h3 class="hometitle"><?php echo $port_full_title; ?></h3></div>
		<?php global $virtue_premium; if(!empty($virtue_premium['home_portfolio_full_order'])) {$hp_orderby = $virtue_premium['home_portfolio_full_order'];} else {$hp_orderby = 'menu_order';}
					$portfolio_item_full_types = $virtue_premium['portfolio_full_show_type'];
					if(isset($virtue_premium['home_port_count'])) {$portfolio_item_count = $virtue_premium['home_port_count'];} else {$portfolio_item_count = '8';}
					if(isset($virtue_premium['home_port_columns'])) {$portfolio_column = $virtue_premium['home_port_columns'];} else {$portfolio_column = '4';}
               ...
               <?php if(isset($virtue_premium['portfolio_full_show_filter']) && $virtue_premium['portfolio_full_show_filter'] == 1) { ?>
            ...
            <?php } ?>
            <div id="portfoliowrapper" class="rowtight init-isotope" data-fade-in="<?php echo $animate;?>" data-iso-selector=".p-item"> 
            <?php 
				$temp = $wp_query; 
				  $wp_query = null; 
				  ...
				  $wp_query->query(array(
					...
					<div class="<?php echo $itemsize;?> all <?php echo strtolower($tax); ?>  p-item">
                    				...
                    				$attachments = array_filter( explode( ',', $image_gallery ) );
                    					if ($attachments) {
											foreach ($attachments as $attachment) {
												...
												?>
												<li><a href="<?php the_permalink() ?>" alt="<?php the_title(); ?>"><img src="<?php echo $image ?>"/></a>
										...
										if ($attachments) {
											foreach ($attachments as $attachment) {
												...
												$image = aq_resize($attachment_url, $slidewidth, $slideheight, true);
													if(empty($image)) {$image = $attachment_url;} ?>
												<li><a href="<?php the_permalink() ?>" alt="<?php the_title(); ?>"><img src="<?php echo $image ?>"/></a>
												<?php if($plb) {?><a href="<?php echo $attachment_url; ?>" class="kad_portfolio_lightbox_link" title="<?php the_title();?>" rel="lightbox"><i class="icon-search"></i></a><?php }?></li>
												<?php
											}
                    					}	
								} ?>  

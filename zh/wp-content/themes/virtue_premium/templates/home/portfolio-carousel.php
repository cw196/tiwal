<div class="home-portfolio home-margin carousel_outerrim home-padding kad-animation" data-animation="fade-in" data-delay="0">
		<?php global $virtue_premium; if(!empty($virtue_premium['portfolio_title'])) {$porttitle = $virtue_premium['portfolio_title']; } else { $porttitle = __('Featured Projects', 'virtue'); } ?>
		<div class="clearfix"><h3 class="hometitle"><?php echo $porttitle; ?></h3></div>
		<?php global $virtue_premium; if(!empty($virtue_premium['home_portfolio_order'])) {$hp_orderby = $virtue_premium['home_portfolio_order'];} else {$hp_orderby = 'menu_order';}
		...
		if(isset($virtue_premium['home_portfolio_carousel_scroll']) && $virtue_premium['home_portfolio_carousel_scroll'] == 'all' ) {$hport_scroll = '';} else {$hport_scroll = 'items:1,';}
                    				...
                    				$attachments = array_filter( explode( ',', $image_gallery ) );
                    					if ($attachments) {
											foreach ($attachments as $attachment) {
												...
												?>
												<li><a href="<?php the_permalink() ?>" class="kad_portfolio_link" alt="<?php the_title(); ?>"><img src="<?php echo $image ?>" width="<?php echo $slidewidth;?>" height="<?php echo $slideheight;?>"/></a></li>
												<?php
											}
										}
                    			} else {
								...
								if (has_post_thumbnail( $post->ID ) ) {
								 ...
								 $image = aq_resize($thumbnailURL, $slidewidth, $slideheight, true); 
									...
									<div class="imghoverclass">
	                                       ...
	                                       <img src="<?php echo $image ?>" alt="<?php the_title(); ?>" width="<?php echo $slidewidth;?>" height="<?php echo $slideheight;?>" class="lightboxhover" style="display: block;">
	                                       </a> 
	                                </div>
